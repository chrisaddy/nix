vim.g.mapleader = ' ' -- Set the global leader key to space, must be set before plugins are loadeidgrinn
...
vim.schedule(function() -- Schedule the clipboard setup to run after Neovim initialization
  vim.opt.clipboard = 'unnamedplus' -- Use the system clipboard for all operations
end)
vim.opt.breakindent = true -- Enable 'breakindent' to preserve indentation when wrapping long lines
vim.opt.undofile = true -- Enable persistent undo, allowing undo history to be saved between sessions
...
vim.opt.signcolumn = 'yes' -- Always show the signcolumn, where signs like git status and diagnostics appear
vim.opt.updatetime = 250 -- Decrease update time to 250 milliseconds for a more responsive UI
vim.opt.timeoutlen = 300 -- Set the timeout for key sequences to 300 milliseconds
